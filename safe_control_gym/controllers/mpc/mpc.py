...
class MPC(BaseController):
    ...
    def reset(self):
        ...
        if self.env.TASK == Task.STABILIZATION:
            ...
            self.traj = np.hstack([self.traj, self.traj])
    ...
    def compute_initial_guess(self, init_state, goal_states=None):
        ...
        elif self.compute_initial_guess_method == 'policy':
            # initialize the guess solutions
            x_guess = np.zeros((self.model.nx, self.T + 1))
            u_guess = np.zeros((self.model.nu, self.T))
            x_guess[:, 0] = init_state
            for i in range(self.T):
                u_guess[:, i] = self.pi_casadi_fn(cs.veccat(x_guess[:, i], goal_states[:, i])).full().squeeze()
                x_guess[:, i + 1, None] = self.dynamics_func(x0=x_guess[:, i], p=u_guess[:, i])['xf'].toarray()
        else:
