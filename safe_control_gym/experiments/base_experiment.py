        ...
        if verbose:
            for metric_key, metric_val in metrics.items():
                if isinstance(metric_val, list) or isinstance(metric_val, np.ndarray):
                    ...
                    print('{}: {}'.format(colored(metric_key, 'yellow'), metric_val))
                else:
    ...
    * episode constraint violation steps
    ...
    def compute_metrics(self, data, verbose=False):
        ...
        metrics = {
            ...
            'average_success': np.mean(self.get_success()),
            ...
            'rmse_std': np.nanstd(self.get_episode_rmse()),
            'worst_case_rmse_at_0.5': compute_cvar(np.asarray(self.get_episode_rmse()), 0.5, lower_range=False),
            'failure_rate': np.nanmean(self.get_episode_constraint_violations()),
            'average_constraint_violation': np.nanmean(self.get_episode_constraint_violation_steps()),
            'constraint_violation_std': np.nanstd(self.get_episode_constraint_violation_steps()),
            ...
            'mean_t_wall_ms': [np.asarray(tw[:, 0]).mean()*1e3 for tw in self.get_t_wall()],
            'median_t_wall_ms': [np.median(np.asarray(tw[:, 0]))*1e3 for tw in self.get_t_wall()],
            'max_t_wall_ms': [np.max(np.asarray(tw[:, 0]))*1e3 for tw in self.get_t_wall()],
            # others ???
        }
        return metrics

    def get_success(self):
        return [False if "solver_failed" in sim[-1].keys() else True for sim in self.data['info']]

    def get_t_wall(self):
        try:
            # t_wall dims (N_sim, 3), columns corresponds to time_tot, time_preparation, time_feedback
            t_wall = []
            for t_wall_k in self.data['controller_data'][0]['t_wall']:
                t_wall.append(t_wall_k)
            return t_wall
        except:
            return [np.atleast_2d([np.nan]*l) for l in self.get_episode_lengths()]
    ...
    def get_episode_data(self, key, postprocess_func=lambda x: x):
        ...
        if key in self.data:
            ...
            episode_data = []
            for s, ep_info in zip(success, self.data['info']):
                if s:
                    ep_info_data = []
                    for info in ep_info:
                        if key in info:
                    ...
                    episode_data.append(np.array(np.nan))
        else:
